...
const OUT_HEADERS = ["Contrato","UF","Tomador","Ação","Abreviatura","UG Emitente","UG Favorecida","Nota de Empenho","Vinculação","Fonte","Categoria","Competência","Valor Original Total","RP","Saldo utilizado NE","Saldo remanescente NE"];
const ALIASES = {
  dhr: {
    operacao:["operacao","operação","operacao contrato","numero operacao"], dv:["dv","digito","dígito"], proposta:["proposta","proposta tgov","prop tgov","prop. tgov"], convenio:["convenio siafi","convênio siafi","convenio","convênio","numero instrumento","número instrumento","n instrumento","no instrumento"], tomador:["tomador","beneficiario","beneficiário","nome proponente","proponente"], municipio:["municipio","município","municipio beneficiado","município beneficiado"], uf:["uf","estado"], necessidade:["valor necessidade financeira","necessidade financeira","valor solicitado","valor da solicitacao","valor da solicitação","valor autorizado rp3","valor da parcela autorizado rp3","valor parcela autorizado rp3"], rp3:["saldo empenhado rp3","valor empenhado rp3","valor autorizado rp3","valor da parcela autorizado rp3"], empenhado:["valor empenhado rp2 e rp3","valor empenhado rp2 rp3","valor empenhado acumulado"]
  },
  siafi: {
    convenio:["numero do convenio","número do convênio","numero convenio","número convênio","convenio siafi","convênio siafi","informacao complementar","informação complementar","ne ccor informacao complementar","ne ccor informação complementar","ne cco informacao complementar"], info:["informacao complementar","informação complementar"], nota:["nota de empenho","ne ccor","ne cco","ne ccor nota","ne cco nota","ne"], fonte:["fonte recursos detalhada","fonte de recursos detalhada","fonte"], vinculacao:["vinculacao provavel","vinculação provável","vinculacao","vinculação"], favorecido:["ne ccor favorecido","ne cco favorecido","favorecido","beneficiario","beneficiário","cnpj favorecido"], processo:["numero processo","número processo","ne ccor num processo","ne ccor núm processo","ne cco num processo","ne cco núm processo","processo"], emp:["despesas empenhadas a liquidar controle emp","despesas empenhadas a liquidar","empenhadas a liquidar","30 despesas empenhadas a liquidar"], liq:["despesas liquidadas a pagar controle empenho","despesas liquidadas a pagar","liquidadas a pagar","32 despesas liquidadas a pagar"], rap:["restos a pagar a pagar proc e n proc","restos a pagar a pagar","restos a pagar","53 restos a pagar"]
  }
};
...
function friendlyKeys(req){const keys=[];if(req.convenioKey)keys.push(`instrumento ${formatContract(req)}`);if(req.propostaKey)keys.push(`proposta ${clean(req.proposta)}`);if(clean(req.tomador))keys.push(`proponente ${clean(req.tomador)}`);if(clean(req.municipio))keys.push(`município ${clean(req.municipio)}`);return keys.join("; ")||"sem chave identificável"}
...
async function process(){
  ...
  for(const required of ["convenio","tomador","uf","necessidade"])if(dhrMap[required]===undefined)throw new Error(`A coluna indispensável '${required}' não foi reconhecida na planilha do DHR.`);
  for(const required of ["nota","fonte","emp","rap"])if(siafiMap[required]===undefined)throw new Error(`A coluna indispensável '${required}' não foi reconhecida na planilha do SIAFI.`);
  ...
  const output=[OUT_HEADERS], log=[["Contrato","Tomador","Valor pendente","Chaves consultadas","Inconsistência"]], used=new Map();let allocations=0;
  for(const req of requests){let remaining=req.valor, found=false;const candidates=orderCandidates(selectCandidates(req,noteList),req.valor);
    if(!candidates.length){addLog(log,formatContract(req),clean(req.tomador),remaining,friendlyKeys(req),"Nenhuma Nota de Empenho vinculável: o SIAFI não apresentou correspondência para as chaves indicadas.");continue;}
    ...
    if(remaining>0.005)addLog(log,formatContract(req),clean(req.tomador),remaining,friendlyKeys(req),found?"Saldo insuficiente nas Notas de Empenho vinculadas.":"Nenhuma Nota de Empenho com saldo disponível.");
  }
  if(!requests.length)addLog(log,"","",0,"coluna de necessidade financeira",`Nenhuma linha com valor positivo foi identificada na coluna “${dhrTable.headers[dhrMap.necessidade]}”.`);
